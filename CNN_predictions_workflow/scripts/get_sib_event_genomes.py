...
with open(snakemake.input["sib_data"]) as file:

    reader = csv.DictReader(file, delimiter="\t")

    for row in reader:

        if row['variant_set'] == 'proband_set':
            event_sibs[(row['event'], row['proband'])].add(row['variant'])
        if row['variant_set'] == 'unaffected_set':
            event_sibs[(row['event'], row['unaffected_sibling'])].add(row['variant'])

...
def main(known_foreground_events, known_background_events, novel_microexons, test_set_event_ids,
         ...
         validation_split, test_chroms):
  known_foreground_events = load_events(known_foreground_events)
  events_bg = load_events(known_background_events, filter_neural_high=False)
  novel_microexons = load_events(novel_microexons, filter_neural_high=False)
  novel_microexons = novel_microexons.loc[~novel_microexons.chrom.isin(test_chroms)]

  test_set_event_ids = pd.read_table(test_set_event_ids, index_col=0, header=None).index

  events_fg = pd.concat([known_foreground_events, novel_microexons])
